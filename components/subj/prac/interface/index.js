...
export default function Interface({ pid }) {
  const { setResult ,result} = useContext(CostumeData);
  const router = useRouter();
  const { subj } = router.query;
  const [answers, setAnswers] = useState([]);
  const [loadMe, setLoadMe] = useState(false);
  const [interme, { data,loading}] = useLazyQuery(Prac_InterFace,{fetchPolicy:"no-cache"});
  const [updatme] = useMutation(Send_Answers, {
    fetchPolicy:"no-cache",
    onCompleted: data => {
      setLoadMe(false);
      const {updateUserProgress} = data 
  ...
  useEffect(() => {
    if(!result || result.pid !== pid || result.subj !== subj){
    setResult(undefined);
    setAnswers([]);
    }
  }, [pid, subj,result]);
